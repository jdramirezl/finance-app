...
- **Movement**
  ...
  - Properties: `type`, `amount`, `accountId`, `pocketId`, `subPocketId` (optional), `notes`, `displayedDate`, `createdAt`, `isPending` (optional)
  - Types: `IngresoNormal`, `EgresoNormal`, `IngresoFijo`, `EgresoFijo`, `InvestmentIngreso`, `InvestmentShares`
  - **Pending Movements**: Can be marked as pending to register without affecting balances (see [PENDING_MOVEMENTS.md](./PENDING_MOVEMENTS.md))
