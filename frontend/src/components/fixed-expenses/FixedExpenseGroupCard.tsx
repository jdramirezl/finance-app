...
function getStatusColor(status: ExpenseStatus): string {
  switch (status) {
    ...
    case 'RECURRING': return 'text-primary';
    case 'IN PROGRESS': return 'text-on-surface-variant';
    case 'DUE SOON': return 'text-error';
...
const FixedExpenseGroupCard = ({
  ...
  return (
    <section className="bg-surface-container/80 backdrop-blur-[12px] border border-white/[0.08] rounded-xl flex flex-col">
      ...
      <div className="p-5 border-b border-white/5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-xl font-semibold text-on-surface">{group.name}</h3>
        </div>
        ...
        <div className="relative" ref={menuRef}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-8 z-20 bg-surface-container-high border border-white/10 rounded-lg shadow-xl py-1 min-w-[140px]">
              {!isDefaultGroup && (
                <>
                  <button
                    onClick={() => { onEditGroup(group); setMenuOpen(false); }}
                    className="w-full flex items-center gap-2 px-4 py-2 text-sm text-on-surface hover:bg-white/5"
                  >
                    <Edit2 className="w-3.5 h-3.5" /> Edit
                  </button>
                  <button
                    onClick={() => { onDeleteGroup(group); setMenuOpen(false); }}
                    className="w-full flex items-center gap-2 px-4 py-2 text-sm text-error hover:bg-white/5"
                  >
            ...
            return (
              ...
              >
                ...
                <div className="h-1 w-full bg-surface-container-highest rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-primary to-[#4cd7f6] rounded-full transition-all duration-700 ease-out"
                    style={{ width: `${Math.min(progress, 100)}%` }}
                  />
                </div>

                {/* Monthly contrib + status badge */}
                <div className="flex justify-between text-[10px] font-bold uppercase tracking-[0.06em] text-on-surface-variant">
                  <span>
      ...
      {/* Footer: group total */}
      <div className="px-5 py-4 bg-white/5 mt-auto rounded-b-xl border-t border-white/5">
        <div className="flex justify-between items-center">
