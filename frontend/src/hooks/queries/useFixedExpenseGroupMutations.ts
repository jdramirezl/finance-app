...
export const useFixedExpenseGroupMutations = () => {
    ...
    const createFixedExpenseGroup = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['fixedExpenseGroups'], exact: true });
        },
        onError: (error) => {
    ...
    const deleteFixedExpenseGroup = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['fixedExpenseGroups'], exact: true });
            queryClient.invalidateQueries({ queryKey: ['subPockets'], exact: true }); // Expenses moved to default group
        },
        onError: (error) => {
