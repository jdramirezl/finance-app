/**
 * Apply Pending Movement Use Case
 * 
 * Business logic for applying a pending movement (setting isPending to false).
 * Recalculates balances for affected entities after applying.
 * 
 * Requirements: 11.1
 */
...
@injectable()
export class ApplyPendingMovementUseCase {
  ...
  async execute(id: string, userId: string): Promise<MovementResponseDTO> {
    ...
    // Verify movement is pending
    if (!movement.isPending) {
      throw new ValidationError('Movement is not pending');
    }

    // Set isPending to false (Requirement 11.1)
    movement.applyPending();

    // Persist changes
    await this.movementRepo.update(movement, userId);

    // Recalculate balances for affected entities (Requirement 11.1)
  ...
  /**
   * Recalculate balances for all affected entities after applying pending movement
   */
