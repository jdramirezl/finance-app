/**
 * Unit tests for UnarchivePocketUseCase
 */

import 'reflect-metadata';
import { UnarchivePocketUseCase } from './UnarchivePocketUseCase';
import type { IPocketRepository } from '../../infrastructure/IPocketRepository';
import { Pocket } from '../../domain/Pocket';
import { NotFoundError } from '../../../../shared/errors/AppError';

describe('UnarchivePocketUseCase', () => {
  let useCase: UnarchivePocketUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new UnarchivePocketUseCase(mockPocketRepo);
  });

  it('should unarchive an existing pocket', async () => {
    ...
    expect(mockPocketRepo.unarchive).toHaveBeenCalledWith(pocketId, userId);
  });
  ...
  it('should throw NotFoundError when pocket does not exist', async () => {
    ...
    expect(mockPocketRepo.unarchive).not.toHaveBeenCalled();
  });
});
