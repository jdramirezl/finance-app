/**
 * Unarchive Pocket Use Case
 *
 * Restores a previously archived pocket by clearing its archived_at
 * timestamp.
 */
...
@injectable()
export class UnarchivePocketUseCase {
  constructor(
   ...
   *
   * @param pocketId - Pocket ID to restore
  ...
  async execute(pocketId: string, userId: string): Promise<void> {
    ...
    await this.pocketRepo.unarchive(pocketId, userId);
  }
}
