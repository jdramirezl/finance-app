...
describe('accountService', () => {
    ...
    describe('createAccount', () => {
        ...
        it('should throw error for duplicate account name + currency', () => {
            accountService.createAccount('Test', '#FF0000', 'USD');

            expect(() => {
                accountService.createAccount('Test', '#00FF00', 'USD');
            }).toThrow('An account with name "Test" and currency "USD" already exists');
        });

        it('should allow same name with different currency', () => {
            accountService.createAccount('Test', '#FF0000', 'USD');
            const account2 = accountService.createAccount('Test', '#00FF00', 'MXN');
    ...
    describe('getAccount', () => {
        it('should retrieve account by ID', () => {
            const created = accountService.createAccount('Test', '#FF0000', 'USD');
            const retrieved = accountService.getAccount(created.id);
    ...
    describe('updateAccount', () => {
        it('should update account name', async () => {
            const account = accountService.createAccount('Old Name', '#FF0000', 'USD');
        ...
        it('should update account color', async () => {
            const account = accountService.createAccount('Test', '#FF0000', 'USD');
        ...
        it('should throw error when updating to duplicate name + currency', async () => {
            const acc1 = accountService.createAccount('Account1', '#FF0000', 'USD');
            accountService.createAccount('Account2', '#00FF00', 'USD');
    ...
    describe('deleteAccount', () => {
        ...
        it('should throw error when deleting account with pockets', async () => {
            ...
            vi.spyOn(pocketService.pocketService, 'getPocketsByAccount').mockReturnValue([
                {
                    id: 'pocket-1',
    ...
    describe('validateAccountUniqueness', () => {
        it('should return true for unique combination', () => {
            accountService.createAccount('Test', '#FF0000', 'USD');
            const isUnique = accountService.validateAccountUniqueness('Test', 'MXN');
            expect(isUnique).toBe(true);
        });

        it('should return false for duplicate combination', () => {
            accountService.createAccount('Test', '#FF0000', 'USD');
            const isUnique = accountService.validateAccountUniqueness('Test', 'USD');
            expect(isUnique).toBe(false);
        });

        it('should exclude specified ID from validation', () => {
            const account = accountService.createAccount('Test', '#FF0000', 'USD');
            const isUnique = accountService.validateAccountUniqueness('Test', 'USD', account.id);
            expect(isUnique).toBe(true);
        });
    });
});
